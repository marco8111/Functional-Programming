...
mapear :: [(String, Char)] -> [String]
mapear [(nomex, carac)] | carac == 'm' = map ("Sr." ++) [nomex]
                        | carac == 'f' = map ("Sra." ++) [nomex]
                        | otherwise = error "caracter invalido"  
sufixo :: [(String, Char)] -> [String]
sufixo [] = []
sufixo (y:ys) = mapear[y] ++ sufixo ys
...
{-8_Construa uma função que auxilia o utilizador na escolha de uma password
